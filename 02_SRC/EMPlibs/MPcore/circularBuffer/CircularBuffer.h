...
public:
	...
	uint16_t remaningSpaceLinear() const;
...
template<class T, uint16_t nElem> inline void CircularBuffer<T, nElem>::reset() {
	head_ = tail_;
}
/// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Puts metod
template<class T, uint16_t nElem> uint16_t CircularBuffer<T, nElem>::put(
		T item) {
	if (isFull())
		return -1;
	buf_[head_] = item;
...
template<class T, uint16_t nElem> uint16_t CircularBuffer<T, nElem>::putArray(
		T *item, uint16_t nItem) {
 ...
 */
template<class T, uint16_t nElem> uint16_t CircularBuffer<T, nElem>::putArray(
		T *item, uint16_t nItem, uint16_t bSize) {
...
void CircularBuffer<T, nElem>::memcpyCb(T *memDestArray, uint16_t localTail,
		uint16_t len) {
...
template<class T, uint16_t nElem> inline uint16_t CircularBuffer<T, nElem>::getHead() const {
	return head_;
}
template<class T, uint16_t nElem> inline T* CircularBuffer<T, nElem>::getHeadPtr() {
	return &buf_[head_];
}
template<class T, uint16_t nElem> inline T CircularBuffer<T, nElem>::readHead() const {
	return buf_[head_];
}

// Get Tail Structure information
template<class T, uint16_t nElem> inline uint16_t CircularBuffer<T, nElem>::getTail() const {
	return tail_;
}
template<class T, uint16_t nElem> inline T* CircularBuffer<T, nElem>::getTailPtr() {
	return &buf_[tail_];
}
template<class T, uint16_t nElem> inline T CircularBuffer<T, nElem>::readTail() const {
...
template<class T, uint16_t nElem> inline bool CircularBuffer<T, nElem>::isEmpty() const {
	return head_ == tail_;
}
...
template<class T, uint16_t nElem> inline uint16_t CircularBuffer<T, nElem>::capacity() const {
	return nElem;
}
 ...
 */
template<class T, uint16_t nElem>
inline uint16_t CircularBuffer<T, nElem>::countSlotBetween(uint16_t localTail,
		uint16_t localHead) const {
...
template<class T, uint16_t nElem> inline uint16_t CircularBuffer<T, nElem>::remaningSpaceLinear() const {
...
template<class T, uint16_t nElem> inline uint16_t CircularBuffer<T, nElem>::headAdd(
		uint16_t len) {
...
template<class T, uint16_t nElem> inline uint16_t CircularBuffer<T, nElem>::tailInc() {
	return tailAdd(1);
}

template<class T, uint16_t nElem> inline uint16_t CircularBuffer<T, nElem>::tailAdd(
		uint16_t len) {
	if (usedSpace() < len)
		return -1;
