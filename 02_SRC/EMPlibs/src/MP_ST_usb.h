...
templatePar() class MP_ST_usb : public MP<templateParCall()> {
public:
...
public:
  ...
  int16_t getData_wait(pIn *pack,
                       unsigned long nTikc); // return the residual pack available after the remove, or -3 for timeout
private:
...
templatePar() MP_ST_usb<templateParCall()>::MP_ST_usb() : MP_ST_usb<templateParCall()>(callBacksMP()) {}

templatePar() MP_ST_usb<templateParCall()>::MP_ST_usb(callBacksMP callBack) : MP<templateParCall()>(callBack) {
  byteSend.memClean();
  instance = this;
...
templatePar() int16_t MP_ST_usb<templateParCall()>::getData_wait(pIn *pack, unsigned long nTikc) {
...
templatePar() void MP_ST_usb<templateParCall()>::packTimeRefresh() { lastDecodeTime = HAL_GetTick(); }

templatePar() unsigned long MP_ST_usb<templateParCall()>::lastPackElapsed() { return HAL_GetTick() - lastDecodeTime; }
} /* namespace EMP */
